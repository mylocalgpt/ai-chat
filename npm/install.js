...
const archiveName = `ai-chat_${mappedPlatform}_${mappedArch}.${archiveExt}`;
const binDir = path.join(__dirname, 'bin');
const binaryName = platform === 'win32' ? 'ai-chat.exe' : 'ai-chat';
const binaryPath = path.join(binDir, binaryName);

const baseUrl = `https://github.com/mylocalgpt/ai-chat/releases/download/v${version}`;
...
async function main() {
  console.log(`Downloading ai-chat v${version} for ${platform}/${arch}...`);
  ...
  if (platform === 'win32') {
    const extractedBinary = path.join(binDir, 'ai-chat.exe');
  ...
  console.log('ai-chat installed successfully.');
}
